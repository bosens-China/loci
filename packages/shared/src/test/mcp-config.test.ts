...
describe('具名 MCP 客户端配置', () => {
  ...
  it('按 VS Code 与 Claude Code 的字段生成配置', () => {
    expect(JSON.parse(createMcpClientConfig('vscode', stdioConnection))).toEqual({
      servers: {
  ...
  it('生成 Google Antigravity stdio 配置', () => {
    expect(JSON.parse(createMcpClientConfig('antigravity', stdioConnection))).toEqual({
