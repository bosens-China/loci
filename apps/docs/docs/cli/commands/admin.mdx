...
`--schedule off` 关闭 Server 自动更新计划；旧脚本中的 `manual` 仍可作为兼容别名。`admin sync` 可以接收多个名称、完整 ID 或唯一短 ID；使用 `--all` 提交全部文档库，使用 `--wait` 在当前终端等待聚合结果。同一文档库重复提交时，Loci 会复用已有任务并显示当前进度。
