...
从任意一个公开文档页创建本地文档源。只提供 URL 时，CLI 会直接使用已保存的安全偏好；首次使用或偏好失效时回退到产品基础默认值，不再逐项询问。URL 是抓取入口，不必是网站首页。
...
首次使用时，该示例会创建名为 `rspress` 的文档源，并采用以下产品基础默认值：
...
CLI 会记住上一次成功创建时的抓取方式、页面上限、收录路径层级和首次同步选择。后续向导和省略对应参数的 `source add <url>` 会使用这些安全偏好；显式命令参数始终优先，失效偏好回退到上表的产品基础默认值。密码、删除目标和危险确认不会被记住。

CLI、桌面和 MCP 共享上述业务字段的基础默认值、取值边界、全局继承和 GitHub 规范化规则，但保留不同交互：CLI 提供提示、确认与前台/后台选项，MCP 提供结构化结果、批量调用、分页和等待开关。CLI 偏好只影响 CLI，不会改变 MCP 省略参数时的行为。

#### GitHub 仓库 Markdown

公开 `github.com/<owner>/<repo>` 仓库会自动切换为 GitHub 文档源，不需要手动选择类型。名称默认使用仓库名，`--page-limit` 表示最多收录的 Markdown 数量；创建和更新输入统一按 `auto` 与根路径规范化，网页抓取方式、路径范围和并发设置不参与 GitHub 同步。
