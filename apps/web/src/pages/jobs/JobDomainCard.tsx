...
export function JobDomainCard(props: JobDomainCardProps): React.JSX.Element {
  ...
  return (
    <section className="panel overflow-hidden">
      <header className="border-b border-[#dce6e5] bg-[#f4f8f7] px-4 py-4 sm:px-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2.5">
              <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-accent shadow-[0_0_0_4px_rgba(10,124,134,.10)]" />
              <h2 className="m-0 truncate font-mono text-sm font-700 text-ink">{group.hostname}</h2>
              <span className="rounded-full bg-[#dcebea] px-2 py-0.5 text-[10px] font-700 tracking-wide text-[#466467] uppercase">
                共享队列
              </span>
            </div>
            <div className="mt-2 grid grid-cols-2 gap-x-5 gap-y-1 text-xs text-muted sm:flex sm:flex-wrap">
      ...
      <div className="max-h-112 divide-y divide-[#e5ecec] overflow-y-auto">
        {group.jobs.map((job) => (
...
function JobRow(props: JobDomainCardProps & { job: LocalJob }): React.JSX.Element {
  ...
  return (
    <article className="px-4 py-4 transition-colors hover:bg-[#fafcfc] sm:px-5">
      <div className="grid gap-4 lg:grid-cols-[minmax(14rem,1.4fr)_minmax(12rem,1fr)_9rem_11rem_auto] lg:items-center">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="truncate font-650 text-ink">
              {props.sourceNames.get(job.sourceId) ?? '文档同步'}
            </span>
            <JobStatus status={status} />
            {job.priority !== 0 && (
              <span className="text-[10px] font-700 text-accent">优先级 {job.priority}</span>
            )}
          </div>
          <div className="mt-1 truncate font-mono text-[11px] text-muted" title={job.id}>
            {job.id}
          </div>
          {job.error && (
            <Tooltip title={job.error} placement="bottomLeft">
              <div className="mt-1.5 truncate text-xs text-[#a33e38]">失败原因：{job.error}</div>
            </Tooltip>
          )}
        </div>
        <div className="min-w-0">
          ...
          {progress?.node && (
            <div className="mt-0.5 truncate text-[11px] text-muted" title={progress.node.url}>
              {progress.node.title}
            </div>
          )}
        </div>
...
function Metric({ label, value }: { label: string; value: string }): React.JSX.Element {
  return (
    <div className="mb-1">
      ...
      <span className="ml-1 font-600 text-ink">{value}</span>
    </div>
  )
}
...
> = {
  pending: { label: '等待中', className: 'bg-[#fff0d7] text-[#8b570f]' },
  running: { label: '运行中', className: 'bg-[#e1f1f3] text-[#086a72]' },
  pausing: { label: '暂停中', className: 'bg-[#fff0d7] text-[#8b570f]' },
  paused: { label: '已暂停', className: 'bg-[#e8ecec] text-[#526568]' },
  stopping: { label: '结束中', className: 'bg-[#fff0d7] text-[#8b570f]' },
  stopped: { label: '已结束', className: 'bg-[#e8ecec] text-[#526568]' },
  completed: { label: '已完成', className: 'bg-[#e4f2eb] text-[#226446]' },
  failed: { label: '失败', className: 'bg-[#fae9e6] text-[#963b36]' },
  cancelled: { label: '已取消', className: 'bg-[#edf0f0] text-[#667476]' }
}
