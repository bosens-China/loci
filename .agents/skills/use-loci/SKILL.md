...
3. 本地存在时，查看浅层目录树，再直接读取目标文件或搜索正文。
...
6. 文档准备完成后，根据目录和搜索结果调用 `loci_read_files` 阅读相关文件或完整小节。
7. 根据读取结果回答，附上相关 `source_url`，并区分文档事实与 Agent 推断。

## 目录与搜索

把目录树作为理解文档库结构的主要入口。获得本地文档库后，先调用 `loci_get_library_tree` 查看一至两层，了解主题、模块、版本和相邻文件，再决定直接读取还是搜索。
