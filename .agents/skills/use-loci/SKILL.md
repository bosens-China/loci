...
调用 `loci_add_library` 时使用产品基础值：`auto`、1000 页、根路径，并让源级并发和 GitHub 大小上限继承全局设置。工具不读取普通 CLI 的最近创建偏好。已知目标文档位于固定路径或用户限制了规模时，直接传 `scope_path`、`page_limit` 等覆盖值。同一域名和同一路径会复用；同一域名的不同路径可以分别建库。GitHub 仓库会统一按 `auto` 和根路径规范化，无需传网页模式或路径范围。
