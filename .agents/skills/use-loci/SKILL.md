---
name: use-loci
description: 使用 Loci MCP 发现、获取、同步、搜索和阅读开发者技术文档，并协调其他文档来源。仅在回答依赖库、框架、SDK、API、CLI、配置、迁移、版本差异或技术错误等文档事实时使用；非文档任务直接选择对应来源。
---
...
本地或云端未命中本身不构成切换条件。只有技术文档范围内的问题在 Loci 不可用、无法确认官方入口且用户没有指定可用来源、用户拒绝官方抓取、已经提供官方抓取选项后所有已授权获取路径仍失败或没有文件，或者获得的 Loci 证据经过合理检索后仍不足时，才使用 Context7、定向网页检索或其他文档来源。非文档任务不经过这条兜底链路。切换前说明满足了哪一项条件；用户明确指定 Context7 等来源时，直接遵循当前请求。
...
调用 `loci_add_library` 时使用产品基础值：`auto`、1000 页、根路径，并让源级并发和 GitHub 大小上限继承全局设置。MCP 不读取 CLI 最近创建偏好。已知目标文档位于固定路径或用户限制了规模时，直接传 `scope_path`、`page_limit` 等覆盖值。同一域名和同一路径会复用；同一域名的不同路径可以分别建库。GitHub 仓库会统一按 `auto` 和根路径规范化，无需传网页模式或路径范围。
...
用户已授权同步，但另一个工具或 Agent 可能已经启动任务时，先用 `library_id` 调用 `loci_get_sync_status`。`sync_status=syncing` 时跟随已有任务，不要并行启动第二份抓取；确实需要在当前调用中等待时，对同一文档库调用 `loci_sync_libraries` 并设置 `wait_for_completion: true`。Loci 会复用当前 MCP 宿主中的任务，跨进程调用则由文档源锁阻止重复写入。
