...
- 同一文档源的重复同步会复用已有任务；同 hostname 文档库共享队列和实时限速，不同 hostname 可并行处理。Web、CLI 和 MCP 均可暂停、恢复、结束、取消和调整优先级。
- 本地与云端先展示文档库卡片，再按需读取目录和正文；ZIP 备份使用带校验索引的拆分 JSON，管理员可用压缩二进制归档发布本地库到 Server。
